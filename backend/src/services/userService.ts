...
export const userService = {
  ...
  async getById(id: number) {
    const user = await prisma.user.findUnique({ where: { id }, include: { student: true }, omit: { passwordHash: true } });
  ...
  async update(id: number, data: any) {
    ...
    return prisma.user.update({ where: { id }, data: updateData, omit: { passwordHash: true } });
  },
};
